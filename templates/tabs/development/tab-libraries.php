...
if ( isset( $_GET['check_updates'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'plugin_boilerplate_check_library_updates' ) ) {
    $monitor->clear_cache();
...
// Get cached updates
foreach ( $plugin_boilerplate_libraries as $slug => $library ) {
    $updates[ $slug ] = $monitor->check_updates( $slug );
}
?>

<div class="wrap plugin-boilerplate-libraries-tab">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2>Bundled Libraries</h2>
        <a href="<?php echo esc_url( add_query_arg( array( 'check_updates' => '1', '_wpnonce' => wp_create_nonce( 'plugin_boilerplate_check_library_updates' ) ) ) ); ?>" class="button button-primary">
    ...
    <table class="wp-list-table widefat fixed striped">
        ...
        <tbody>
            <?php foreach ( $plugin_boilerplate_libraries as $slug => $library ) : 
                $update = $updates[ $slug ];
            ...
            <tr class="plugin-boilerplate-release-notes" style="display: none;" data-library="<?php echo esc_attr( $slug ); ?>">
...
<style>
.plugin-boilerplate-libraries-tab .wp-list-table td {
    vertical-align: top;
    padding: 12px 10px;
}
.plugin-boilerplate-libraries-tab .button-small {
    height: auto;
    padding: 4px 8px;
    font-size: 12px;
}
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.plugin-boilerplate-dismiss-library-notice', function(e) {
        e.preventDefault();
        $.post(ajaxurl, {
            action: 'plugin_boilerplate_dismiss_library_notice'
        });
