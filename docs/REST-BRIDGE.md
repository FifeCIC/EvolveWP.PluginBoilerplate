...
│       'capability' => 'plugin_boilerplate_manage_settings',              │
│       'label'      => 'List Projects',                       │
│   ) );                                                       │
└──────────────┬──────────────────────────────────────────────┘
               │
...
│   Permission factory ───────── plugin_boilerplate_user_can() checks      │
...
```bash
# Test connection
curl -X GET https://example.com/wp-json/plugin-boilerplate/v1/connector/github/test \
  -H "X-WP-Nonce: {nonce}"

# List capabilities
curl -X GET https://example.com/wp-json/plugin-boilerplate/v1/connector/github/capabilities \
  -H "X-WP-Nonce: {nonce}"

# Execute an action
curl -X POST https://example.com/wp-json/plugin-boilerplate/v1/connector/github/create_issue \
  -H "X-WP-Nonce: {nonce}" \
  -H "Content-Type: application/json" \
  -d '{"repo": "evolvewp/plugin-boilerplate", "title": "Bug fix", "body": "Details..."}'
```
...
```
Q → curl → /wp-json/plugin-boilerplate/v1/connector/github/create_issue
...
```php
$all = plugin_boilerplate_rest_endpoints();

// Filter by source
$manual    = plugin_boilerplate_rest_endpoints( 'manual' );
$connector = plugin_boilerplate_rest_endpoints( 'connector' );
```
...
### plugin_boilerplate_rest_bridge_before_register
...
```php
add_action( 'plugin_boilerplate_rest_bridge_before_register', function() {
    REST_Bridge::register( 'wpseed/v1', '/late-endpoint', array( ... ) );
} );
```

### plugin_boilerplate_rest_bridge_registered

Fires after all routes are registered. Receives the full endpoint metadata array.

```php
add_action( 'plugin_boilerplate_rest_bridge_registered', function( $endpoints ) {
...
| `functions.php` | `plugin_boilerplate_rest_endpoints()` global accessor |
...
2. The namespace changes (e.g. `wpseed/v1` → `plugin-boilerplate/v1`).
3. The `plugin_boilerplate_rest_endpoints()` function becomes `yourprefix_rest_endpoints()`.
