...
- `$group` (string) - Group name for organization (default: 'plugin-boilerplate')
...
- `$group` (string) - Group name (default: 'plugin-boilerplate')
...
add_action('daily_cleanup', function() {
    // Delete old logs
    global $wpdb;
    $wpdb->query("DELETE FROM {$wpdb->prefix}plugin_boilerplate_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
});
```
...
// ✅ Good - specific name
$scheduler->schedule_single('plugin_boilerplate_process_user_data', $args);
```
