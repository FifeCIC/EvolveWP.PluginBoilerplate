...
as_schedule_single_action( time() + 3600, 'plugin_boilerplate_process_data', array( 'user_id' => 123 ) );

// Schedule a recurring action (every hour)
as_schedule_recurring_action( time(), HOUR_IN_SECONDS, 'plugin_boilerplate_hourly_sync' );

// Hook your callback
add_action( 'plugin_boilerplate_process_data', 'plugin_boilerplate_handle_data_processing' );
function plugin_boilerplate_handle_data_processing( $user_id ) {
    // Your processing logic here
    error_log( 'Processing data for user: ' . $user_id );
}

// Check if action is scheduled
if ( as_next_scheduled_action( 'plugin_boilerplate_process_data' ) ) {
    // Action is already scheduled
}

// Cancel scheduled action
as_unschedule_action( 'plugin_boilerplate_process_data', array( 'user_id' => 123 ) );
```
...
add_action( 'after_setup_theme', 'plugin_boilerplate_load_carbon_fields' );
function plugin_boilerplate_load_carbon_fields() {
    \Carbon_Fields\Carbon_Fields::boot();
}

// Register fields
add_action( 'carbon_fields_register_fields', 'plugin_boilerplate_register_custom_fields' );
function plugin_boilerplate_register_custom_fields() {
    
    // Theme Options Page
    Container::make( 'theme_options', __( 'Theme Settings' ) )
        ->add_fields( array(
            Field::make( 'text', 'plugin_boilerplate_site_tagline', 'Site Tagline' ),
            Field::make( 'textarea', 'plugin_boilerplate_footer_text', 'Footer Text' ),
            Field::make( 'image', 'plugin_boilerplate_logo', 'Logo' ),
        ) );
    ...
    Container::make( 'post_meta', __( 'Custom Fields' ) )
        ...
        ->add_fields( array(
            Field::make( 'text', 'plugin_boilerplate_subtitle', 'Subtitle' ),
            Field::make( 'checkbox', 'plugin_boilerplate_featured', 'Featured Post' ),
            Field::make( 'complex', 'plugin_boilerplate_gallery', 'Gallery' )
                ->add_fields( array(
    ...
    Container::make( 'user_meta', __( 'Social Profiles' ) )
        ->add_fields( array(
            Field::make( 'text', 'plugin_boilerplate_twitter', 'Twitter URL' ),
            Field::make( 'text', 'plugin_boilerplate_linkedin', 'LinkedIn URL' ),
        ) );
}

// Retrieve values
$tagline = carbon_get_theme_option( 'plugin_boilerplate_site_tagline' );
$subtitle = carbon_get_post_meta( get_the_ID(), 'plugin_boilerplate_subtitle' );
$twitter = carbon_get_user_meta( $user_id, 'plugin_boilerplate_twitter' );
```
...
```php
require_once PLUGIN_BOILERPLATE_PLUGIN_DIR_PATH . 'includes/libraries/parsedown/Parsedown.php';
...
```php
// Schedule action
as_enqueue_async_action( 'plugin_boilerplate_my_process', array( 'id' => 1 ) );

// Hook callback
add_action( 'plugin_boilerplate_my_process', 'plugin_boilerplate_process_item' );
function plugin_boilerplate_process_item( $id ) {
    // Process item
}
```
...
**Old Code:**
```php
add_settings_section( 'plugin_boilerplate_section', 'Settings', null, 'plugin-boilerplate' );
add_settings_field( 'plugin_boilerplate_api_key', 'API Key', 'plugin_boilerplate_api_key_callback', 'plugin-boilerplate', 'plugin_boilerplate_section' );
register_setting( 'plugin-boilerplate', 'plugin_boilerplate_api_key' );
```
...
Container::make( 'theme_options', 'Settings' )
    ->add_fields( array(
        Field::make( 'text', 'plugin_boilerplate_api_key', 'API Key' ),
    ) );
```
