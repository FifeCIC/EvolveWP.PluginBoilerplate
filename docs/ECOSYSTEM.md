...
add_action('plugin_boilerplate_ecosystem_register', function() {
    plugin_boilerplate_ecosystem()->register_plugin('myplugin', array(
        'name' => 'My Plugin',
        'version' => '1.0.0',
...
plugin_boilerplate_ecosystem()->register_shared_resource('logging', function($plugin_slug) {
...
plugin_boilerplate_ecosystem()->register_shared_resource('background_tasks', function() {
...
```php
if (plugin_boilerplate_ecosystem()->is_ecosystem_mode()) {
    // 2+ plugins installed
    // Use shared menus
} else {
    // Single plugin
...
```php
// In your plugin
plugin_boilerplate_ecosystem()->register_shared_resource('logging', 'myplugin_render_logs', 10);
...
add_filter('plugin_boilerplate_ecosystem_available_plugins', function($plugins) {
    $plugins['myplugin'] = array(
        'name' => 'My Plugin',
        'description' => 'Does amazing things',
        'download_url' => 'https://github.com/user/myplugin/archive/main.zip',
        'required_by' => array(),
        'integrates_with' => array('plugin-boilerplate', 'plugin-boilerplate'),
    );
    return $plugins;
});
```
...
if (plugin_boilerplate_ecosystem()->is_registered('plugin-boilerplate')) {
    // wpseed is installed
    // Enable plugin-boilerplate-specific features
}

// Get all installed plugins
$plugins = plugin_boilerplate_ecosystem()->get_plugins();
...
```php
// In your plugin
add_action('plugin_boilerplate_ecosystem_plugin_registered', function($slug, $plugin) {
    if ($slug === 'plugin-boilerplate') {
        // wpseed was just registered
        // Enable wpseed integration features
        update_option('myplugin_plugin_boilerplate_integration', true);
    }
}, 10, 2);
```
...
```php
add_action('plugin_boilerplate_ecosystem_register', 'myplugin_register_ecosystem');
```
...
add_action('admin_menu', function() {
    if (plugin_boilerplate_ecosystem()->use_shared_menu()) {
...
add_action('plugin_boilerplate_ecosystem_plugin_registered', function($slug, $plugin) {
    if ($slug === 'plugin-boilerplate') {
        ...
        update_option('myplugin_plugin_boilerplate_enabled', true);
    }
}, 10, 2);
```
...
```php
// Debug
var_dump(plugin_boilerplate_ecosystem()->is_ecosystem_mode());
var_dump(plugin_boilerplate_ecosystem()->get_plugins());
var_dump(get_option('plugin_boilerplate_ecosystem_menu_location'));
```
...
```php
// Debug
var_dump(plugin_boilerplate_ecosystem()->get_shared_resources('logging'));
```
...
- GitHub: [Report Issue](https://github.com/ryanbayne/plugin-boilerplate/issues)
- Documentation: [Wiki](https://github.com/ryanbayne/plugin-boilerplate/wiki)
