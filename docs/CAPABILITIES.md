...
Capability_Manager::register( 'plugin_boilerplate_manage_settings', array(
    'label'       => __( 'Manage Settings', 'plugin-boilerplate' ),
    'description' => __( 'Access and modify the plugin settings page.', 'plugin-boilerplate' ),
...
if ( plugin_boilerplate_user_can( 'plugin_boilerplate_manage_settings' ) ) {
    // show settings page
}

// Check for a specific user.
if ( plugin_boilerplate_user_can( 'plugin_boilerplate_view_logs', $user_id ) ) {
    // show logs for this user
}

// Using the class directly.
if ( Capability_Manager::user_can( 'plugin_boilerplate_manage_settings' ) ) {
    // same thing
}
```
...
| `plugin_boilerplate_manage_settings` | core | administrator | Access the settings page |
| `plugin_boilerplate_view_development` | development | administrator | Access development tools |
| `plugin_boilerplate_manage_connectors` | api | administrator | Configure API credentials |
| `plugin_boilerplate_execute_connectors` | api | administrator | Run connector actions |
| `plugin_boilerplate_view_logs` | development | administrator | View logs and debug output |

When cloning WPSeed to a new plugin, these are renamed with the plugin's prefix
(e.g. `plugin_boilerplate_manage_settings`).
...
// Returns: array( 'plugin_boilerplate_manage_settings' => array( 'label' => ..., ... ), ... )
```
...
```php
$cap = Capability_Manager::get( 'plugin_boilerplate_manage_settings' );
...
add_filter( 'plugin_boilerplate_user_can', function( $result, $capability, $user_id ) {
    if ( 42 === $user_id ) {
...
### plugin_boilerplate_capabilities_installed

Fires after capabilities are added to roles during activation.

```php
add_action( 'plugin_boilerplate_capabilities_installed', function( $capabilities ) {
    // Log which capabilities were installed.
    error_log( 'Installed ' . count( $capabilities ) . ' capabilities.' );
} );
```

### plugin_boilerplate_capabilities_uninstalled
...
2. The global accessor becomes `plugin_boilerplate_user_can()`.
3. All default capability names are renamed (e.g. `plugin_boilerplate_manage_settings`).
