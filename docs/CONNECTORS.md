...
$issues = $github->execute( 'get_issues', array(
    'repo'  => 'evolvewp/plugin-boilerplate',
    'state' => 'open',
) );
...
```
plugin_boilerplate_api_{provider_id}_key       → API key / access token
plugin_boilerplate_api_{provider_id}_secret    → API secret (if needed)
plugin_boilerplate_api_{provider_id}_url       → Base URL (if configurable)
```
...
```
plugin_boilerplate_api_{provider_id}_{account_id}_key
plugin_boilerplate_api_{provider_id}_{account_id}_secret
plugin_boilerplate_api_{provider_id}_{account_id}_url
```
...
// Named account.
$github_org = plugin_boilerplate_connector( 'github', 'my-org' );
```

### Injecting Credentials from Other Sources

Use the `plugin_boilerplate_connector_credentials` filter to load credentials from
environment variables, a secrets manager, or any other source:

```php
add_filter( 'plugin_boilerplate_connector_credentials', function( $args, $provider_id, $account_id ) {
...
### plugin_boilerplate_api_providers

Filter the complete list of registered connector providers.

```php
add_filter( 'plugin_boilerplate_api_providers', function( $providers ) {
    // Remove a provider.
...
### plugin_boilerplate_connector_credentials
...
### plugin_boilerplate_connector_request_args

Filter the `wp_remote_request()` arguments before every API call.

```php
add_filter( 'plugin_boilerplate_connector_request_args', function( $args, $url, $provider_id, $endpoint ) {
    if ( 'github' === $provider_id ) {
...
| `functions.php` | `plugin_boilerplate_connector()` global accessor |
...
The `plugin_boilerplate_connector()` function becomes `yourprefix_connector()` and
all option names change from `plugin_boilerplate_api_*` to `yourprefix_api_*`.
