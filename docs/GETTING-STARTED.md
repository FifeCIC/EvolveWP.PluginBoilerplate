...
1. Download the latest release from [GitHub](https://github.com/ryanbayne/plugin-boilerplate/releases)
2. Extract to `wp-content/plugins/`
3. Rename folder from `plugin-boilerplate-main` to `wpseed`

### Method 3: WP-CLI
```bash
wp plugin install https://github.com/ryanbayne/plugin-boilerplate/archive/main.zip --activate
```
...
- **Command Line**: 
  ```bash
  # Linux/Mac
  find . -type f -name "*.php" -exec sed -i 's/plugin-boilerplate/myawesomeplugin/g' {} +
  
  # Windows (PowerShell)
  Get-ChildItem -Recurse -Filter *.php | ForEach-Object { (Get-Content $_.FullName) -replace 'plugin-boilerplate', 'myawesomeplugin' | Set-Content $_.FullName }
  ```
...
- [GitHub Discussions](https://github.com/ryanbayne/plugin-boilerplate/discussions)
- [Documentation](https://github.com/ryanbayne/plugin-boilerplate/wiki)
...
- `plugin_boilerplate_get_option()` - Get plugin option
- `plugin_boilerplate_help_tip()` - Add tooltip
- `plugin_boilerplate_clean()` - Sanitize data

### Hooks
- `plugin_boilerplate_loaded` - Plugin loaded
- `plugin_boilerplate_init` - Plugin initialized
- `plugin_boilerplate_settings_save_{tab}` - Settings saved
- `plugin_boilerplate_admin_mainviews` - Add admin views
