...
class EvolveWP_Boilerplate_API_Factory {
	 ...
	 *
	 * @since 3.1.0
	 * @var   array<string, \EvolveWP\PluginBoilerplate\API\Connector_Interface>
	 */
	...
	public static function create( $provider_id, $args = array() ) {
		...
		if ( ! $provider ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Unknown API provider: %s', 'plugin-boilerplate' ),
					$provider_id
				)
			);
		}
		...
		if ( ! empty( $provider['class_path'] ) && ! class_exists( $provider['class_name'], false ) ) {
			$class_file = PLUGIN_BOILERPLATE_PLUGIN_DIR_PATH . 'api/' . $provider['class_path'];
			...
			if ( ! file_exists( $class_file ) ) {
				return new \WP_Error(
					...
					sprintf(
						...
						__( 'Connector class file not found: %1$s (provider: %2$s)', 'plugin-boilerplate' ),
						$provider['class_path'],
						$provider_id
					)
				);
		...
		if ( ! class_exists( $provider['class_name'] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Connector class not found: %1$s (provider: %2$s)', 'plugin-boilerplate' ),
					$provider['class_name'],
					$provider_id
				)
			);
		...
		try {
			...
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to create %1$s connector: %2$s', 'plugin-boilerplate' ),
					$provider_id,
					$e->getMessage()
				)
			);
		...
		if ( ! ( $instance instanceof \EvolveWP\PluginBoilerplate\API\Connector_Interface ) ) {
			return new \WP_Error(
				'invalid_connector',
				sprintf(
					/* translators: 1: class name, 2: interface name */
					__( '%1$s must implement %2$s.', 'plugin-boilerplate' ),
					$provider['class_name'],
					'EvolveWP\Core\\API\\Connector_Interface'
				)
			);
		}
	 ...
	 *   plugin_boilerplate_api_{provider_id}_{field}           (default account)
	 *   plugin_boilerplate_api_{provider_id}_{account_id}_{field}  (named account)
	 *
	 * @since  1.0.0
	...
	public static function create_from_settings( $provider_id, $account_id = '' ) {
		...
		$prefix = empty( $account_id )
			? "plugin_boilerplate_api_{$provider_id}"
			: "plugin_boilerplate_api_{$provider_id}_{$account_id}";
		 ...
		 */
		$args = apply_filters( 'plugin_boilerplate_connector_credentials', $args, $provider_id, $account_id );
