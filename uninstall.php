...
$wpdb->query(
    $wpdb->prepare(
        ...
        $wpdb->esc_like( 'plugin_boilerplate_' ) . '%'
    )
);
...
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_plugin_boilerplate_' ) . '%'
    )
);
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_timeout_plugin_boilerplate_' ) . '%'
    )
);
...
wp_clear_scheduled_hook( 'plugin_boilerplate_daily_cleanup' );
