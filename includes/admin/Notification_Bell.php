...
class Notification_Bell {
	...
	public static function add_notification_bell( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_node( array(
			...
			'meta'  => array( 'class' => 'plugin-boilerplate-notification-bell' ),
		) );
	}
	...
	private static function get_bell_html( $count ) {
		...
		if ( $count > 0 ) {
			$html .= '<span class="plugin-boilerplate-notification-count">' . (int) $count . '</span>';
		}
		return $html;
	}
	...
	public static function enqueue_assets() {
		wp_add_inline_style( 'admin-bar', '
			#wp-admin-bar-plugin-boilerplate-notifications .ab-icon { font-size: 20px; margin-top: 2px; }
			.plugin-boilerplate-notification-count { display: inline-block; background: #d63638; color: #fff; border-radius: 10px; padding: 2px 6px; font-size: 11px; font-weight: 600; margin-left: 5px; line-height: 1; }
			#wp-admin-bar-plugin-boilerplate-notifications:hover .ab-icon { color: #00a0d2; }
		' );
	}
}
