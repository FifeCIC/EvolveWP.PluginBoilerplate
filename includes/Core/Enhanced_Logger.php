 ...
 *   Output → {prefix}plugin_boilerplate_debug_logs DB table
 *
...
class Enhanced_Logger {
	...
	public function save_logs() {
		...
		$table     = $wpdb->prefix . 'plugin_boilerplate_debug_logs';
		$cache_key = 'plugin_boilerplate_logger_table_exists';
		$exists    = wp_cache_get( $cache_key, 'plugin_boilerplate_logger' );

		if ( false === $exists ) {
			$exists = ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) === $table );
			wp_cache_set( $cache_key, $exists, 'plugin_boilerplate_logger', 3600 );
		}
	...
	public static function get_recent_logs( $limit = 50 ) {
		...
		$table     = $wpdb->prefix . 'plugin_boilerplate_debug_logs';
		$safe      = esc_sql( $table );
		$cache_key = 'plugin_boilerplate_recent_logs_' . absint( $limit );
		$cached    = wp_cache_get( $cache_key, 'plugin_boilerplate_logger' );
		...
		wp_cache_set( $cache_key, $results, 'plugin_boilerplate_logger', 60 );
		return $results;
	}
	...
	public static function clear_old_logs( $days = 7 ) {
		...
		wp_cache_delete( 'plugin_boilerplate_recent_logs_50', 'plugin_boilerplate_logger' );
		wp_cache_delete( 'plugin_boilerplate_logger_table_exists', 'plugin_boilerplate_logger' );
...
register_activation_hook( PLUGIN_BOILERPLATE_PLUGIN_FILE, array( __NAMESPACE__ . '\\Enhanced_Logger', 'create_table' ) );
