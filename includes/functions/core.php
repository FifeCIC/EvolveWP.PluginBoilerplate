 ...
 * @return bool
 */
function plugin_boilerplate_is_ajax() {          
...
* @return bool
*/
function plugin_boilerplate_is_https() {      
...
* function instead.
*/
function plugin_boilerplate_is_dashboard() {      
    global $pagenow;
...
*/
function plugin_boilerplate_is_background_process() {        
 ...
 */
function plugin_boilerplate_print_js() {
    global $plugin_boilerplate_queued_js;

    if ( ! empty( $plugin_boilerplate_queued_js ) ) {
        // Sanitize.
        $plugin_boilerplate_queued_js = wp_check_invalid_utf8( $plugin_boilerplate_queued_js );
         ...
         */
        echo wp_kses( apply_filters( 'plugin_boilerplate_queued_js', $js ), array(
            'script' => array( 'type' => array() )
        ) );

        unset( $plugin_boilerplate_queued_js );
    }
}
 ...
 * @return string
 */
function plugin_boilerplate_help_tip( $tip, $allow_html = false ) {
    if ( $allow_html ) {
        $tip = plugin_boilerplate_sanitize_tooltip( $tip );
    } else {
        $tip = esc_attr( $tip );
    }

    return '<span class="plugin-boilerplate-help-tip" data-tip="' . $tip . '"></span>';
}
 ...
 * @param string $code
 */
function plugin_boilerplate_enqueue_js( $code ) {
    global $plugin_boilerplate_queued_js;

    if ( empty( $plugin_boilerplate_queued_js ) ) {
        $plugin_boilerplate_queued_js = '';
    }
 ...
 * @return bool
 */
function plugin_boilerplate_is_developer_mode() {
    return defined('PLUGIN_BOILERPLATE_DEV_MODE') && PLUGIN_BOILERPLATE_DEV_MODE;
}