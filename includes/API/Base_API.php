...
abstract class Base_API implements Connector_Interface {
	...
	public function execute( $action, $params = array() ) {
		...
		if ( ! isset( $capabilities[ $action ] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Action "%1$s" is not supported by the %2$s connector.', 'plugin-boilerplate' ),
					$action,
					$this->provider_id
				),
				array( 'status' => 400 )
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Action "%1$s" is declared but not implemented. Override execute() in %2$s.', 'plugin-boilerplate' ),
				$action,
				static::class
			),
			array( 'status' => 501 )
	...
	protected function make_request( $endpoint, $params = array(), $method = 'GET' ) {
		 ...
		 */
		$args = apply_filters( 'plugin_boilerplate_connector_request_args', $args, $url, $this->provider_id, $endpoint );
	...
	protected function connection_success( $message = '', $data = array() ) {
		if ( empty( $message ) ) {
			$message = sprintf(
				...
				__( 'Successfully connected to %s.', 'plugin-boilerplate' ),
				$this->provider_id
			);
		}
	...
	protected function connection_failure( $message = '', $data = array() ) {
		if ( empty( $message ) ) {
			$message = sprintf(
				/* translators: %s: provider ID */
				__( 'Failed to connect to %s.', 'plugin-boilerplate' ),
				$this->provider_id
			);
		}
