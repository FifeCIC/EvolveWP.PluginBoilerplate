...
class EvolveWP_Core_Library_Update_Monitor {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_plugin_boilerplate_dismiss_library_notice', array( $this, 'ajax_dismiss_notice' ) );
    }
    ...
    public function check_updates( $slug ) {
        ...
        $transient_key = 'plugin_boilerplate_library_update_' . $slug;
    ...
    public function check_outdated_libraries() {
        ...
        $dismissed = get_user_meta( get_current_user_id(), 'plugin_boilerplate_library_notice_dismissed', true );
    ...
    public function clear_cache( $slug = null ) {
        if ( $slug ) {
            delete_transient( 'plugin_boilerplate_library_update_' . $slug );
        } else {
            foreach ( $this->libraries as $slug => $library ) {
                delete_transient( 'plugin_boilerplate_library_update_' . $slug );
            }
    ...
    public function ajax_dismiss_notice() {
        update_user_meta( get_current_user_id(), 'plugin_boilerplate_library_notice_dismissed', time() );
