...
class EvolveWP_Boilerplate_Extension_Installer {
    ...
    public function __construct( $server_url ) {
        ...
        add_action( 'wp_ajax_plugin_boilerplate_install_extension', array( $this, 'ajax_install_extension' ) );
    }
    ...
    public function get_extensions( $license_key = '' ) {
        $response = wp_remote_get( $this->server_url . 'wp-json/plugin-boilerplate-extensions/v1/list', array(
            'body' => array(
    ...
    private function get_download_url( $extension_slug, $license_key = '' ) {
        $response = wp_remote_post( $this->server_url . 'wp-json/plugin-boilerplate-extensions/v1/download', array(
            'body' => array(
        ...
        if ( ! isset( $data['download_url'] ) ) {
            return new WP_Error( 'no_download_url', $data['message'] ?? __( 'Could not get download URL.', 'plugin-boilerplate' ) );
        }
    ...
    public function handle_install() {
        if ( ! isset( $_GET['plugin_boilerplate_install_extension'] ) ) {
            return;
        }

        check_admin_referer( 'plugin_boilerplate_install_extension' );

        if ( ! current_user_can( 'install_plugins' ) ) {
            wp_die( esc_html__( 'You do not have permission to install plugins.', 'plugin-boilerplate' ) );
        }

        $extension_slug = sanitize_text_field( wp_unslash( $_GET['plugin_boilerplate_install_extension'] ) );
        ...
        wp_safe_redirect( admin_url( 'plugins.php?plugin_boilerplate_extension_installed=1' ) );
        exit;
    }
    ...
    public function ajax_install_extension() {
        check_ajax_referer( 'plugin_boilerplate_install_extension', 'nonce' );

        if ( ! current_user_can( 'install_plugins' ) ) {
            wp_send_json_error( array( 'message' => __( 'Permission denied.', 'plugin-boilerplate' ) ) );
        }

        if ( empty( $_POST['extension'] ) ) {
            wp_send_json_error( array( 'message' => __( 'Extension slug is required.', 'plugin-boilerplate' ) ) );
        }
        ...
        wp_send_json_success( array( 'message' => __( 'Extension installed successfully!', 'plugin-boilerplate' ) ) );
    }
