...
class EvolveWP_Boilerplate_Listener {
    ...
    private function process_post_requests() {
        if ( ! isset( $_POST['plugin_boilerplate_form_action'] ) ) {
            return;
        }
        ...
        $action = sanitize_key( $_POST['plugin_boilerplate_form_action'] );
        ...
        do_action( 'plugin_boilerplate_process_form_' . $action );
    }
    
    public function display_notices() {
        $notice = get_transient( 'plugin_boilerplate_admin_notice' );
        
        if ( $notice ) {
            echo '<div class="notice notice-' . esc_attr( $notice['type'] ) . ' is-dismissible"><p>' . 
                 wp_kses_post( $notice['message'] ) . '</p></div>';
            delete_transient( 'plugin_boilerplate_admin_notice' );
        }
    }
}
