...
class EvolveWP_Core_Library_Manager {
    ...
    public function maybe_check_updates() {
        $last_check = get_transient('plugin_boilerplate_library_check');
        
        if (false === $last_check) {
            $this->check_all_updates();
            set_transient('plugin_boilerplate_library_check', time(), DAY_IN_SECONDS);
        }
    }
    ...
    private function check_library_update($id, $library) {
        ...
        if ($latest && version_compare($latest, $library['version'], '>')) {
            update_option('plugin_boilerplate_library_update_' . $id, array(
                'current' => $library['version'],
                'latest' => $latest,
                'checked' => current_time('mysql'),
            ));
        } else {
            delete_option('plugin_boilerplate_library_update_' . $id);
        }
    }
    ...
    public static function get_library_status($id) {
        ...
        $update = get_option('plugin_boilerplate_library_update_' . $id);
        
        // Check if library exists
        $path = PLUGIN_BOILERPLATE_PLUGIN_DIR_PATH . $library['path'] . '/' . $library['file'];
