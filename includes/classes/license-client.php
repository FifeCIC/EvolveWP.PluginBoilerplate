...
class EvolveWP_Boilerplate_License_Client {
    ...
    private $option_name = 'plugin_boilerplate_license_data';
    
    public function __construct() {
        $this->license_server_url = defined('PLUGIN_BOILERPLATE_LICENSE_SERVER') ? PLUGIN_BOILERPLATE_LICENSE_SERVER : '';
        $this->plugin_slug = 'plugin-boilerplate';
        $this->plugin_version = PLUGIN_BOILERPLATE_VERSION;
    ...
    public function check_license_status() {
        ...
        // Check once per day
        $last_check = get_transient('plugin_boilerplate_license_check');
        if ($last_check) {
            return;
        }
        ...
        set_transient('plugin_boilerplate_license_check', time(), DAY_IN_SECONDS);
    }
    ...
    public function check_for_updates($transient) {
        ...
        if (isset($response['new_version']) && version_compare($this->plugin_version, $response['new_version'], '<')) {
            $plugin_file = plugin_basename(PLUGIN_BOILERPLATE_PLUGIN_DIR_PATH . 'plugin-boilerplate.php');
    ...
    private function api_request($endpoint, $data = array()) {
        ...
        $url = trailingslashit($this->license_server_url) . 'wp-json/plugin-boilerplate-license/v1/' . $endpoint;
