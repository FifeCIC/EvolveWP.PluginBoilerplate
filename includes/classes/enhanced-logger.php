...
class EvolveWP_Core_Enhanced_Logger {
    ...
    public function save_logs() {
        ...
        $table_exists_cache_key = 'plugin_boilerplate_logger_table_exists';
        $table_exists = wp_cache_get( $table_exists_cache_key, 'plugin_boilerplate_logger' );
        if ( false === $table_exists ) {
            $table_exists = ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) === $table );
            wp_cache_set( $table_exists_cache_key, $table_exists, 'plugin_boilerplate_logger', 3600 );
        }
        if ( ! $table_exists ) {
            self::create_table();
        }
