...
- [Report Issues](https://github.com/ryanbayne/plugin-boilerplate/issues)
- [Documentation](https://github.com/ryanbayne/plugin-boilerplate/tree/main/docs)
- [Discussions](https://github.com/ryanbayne/plugin-boilerplate/discussions)
...
2. Rename folder from plugin-boilerplate" to "your-plugin-name"
3. Find & Replace: plugin-boilerplate" → "yourplugin", "WPSeed" → "YourPlugin", "WPSEED" → "YOURPLUGIN"
...
* FIXED: W-1d881130 plugin_boilerplate_db_selectwhere() in includes/functions/database.php — direct query already resolved in full rewrite; issue closed via WPVerifier UI
...
* FIXED: readme.txt Stable tag updated from 1.1.0 to 2.0.0 to match Version in plugin-boilerplate.php header, resolving stable_tag_mismatch error
* FIXED: readme.txt Requires at least updated from 5.0 to 4.4 to match plugin-boilerplate.php header, resolving readme_mismatched_header_requires error
...
* FIXED: $_tests_dir in tests/bootstrap.php renamed to $plugin_boilerplate_tests_dir throughout to satisfy WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
...
* FIXED: admin/page/development/development-tabs.php — get_current_tab() now verifies a wp_nonce_url()-signed nonce (action: plugin_boilerplate_dev_tab_navigation) before reading $_GET['tab'], properly satisfying WordPress.Security.NonceVerification.Recommended; nonce_action() helper added for consistency; tab URLs in tabs() updated to include the nonce via wp_nonce_url(); file header and class @version bumped to 2.0.0
...
* FIXED: examples/accordion-table-example.php — $_GET['configure'] gated behind current_user_can(); foreach loop variables renamed from $item/$item_id to $plugin_boilerplate_item/$plugin_boilerplate_item_id; sidebar $selected_item/$item bug fixed to use $plugin_boilerplate_selected_item/$plugin_boilerplate_item
* FIXED: examples/accordion-table-example.php — $_GET['configure'] now verified with wp_verify_nonce() (action: plugin_boilerplate_accordion_configure) before use, properly satisfying WordPress.Security.NonceVerification.Recommended; plugin_boilerplate_accordion_nonce_action() helper added; configure URLs in the item loop now signed with wp_nonce_url(); file @version bumped to 2.0.0
...
* FIXED: examples/integrations/woocommerce-example.php — combined isset( $_POST['_plugin_boilerplate_custom_field'], $_POST['_plugin_boilerplate_custom_field_nonce'] ) split into two separate isset() checks so $_POST['_plugin_boilerplate_custom_field_nonce'] is only ever referenced at the point of immediate sanitised extraction, satisfying WordPress.Security.ValidatedSanitizedInput.InputNotSanitized on line 54; nonce verification restructured to early-return pattern; field value extracted into $plugin_boilerplate_field_value before update_post_meta(); class @version bumped to 2.0.0
...
* FIXED: includes/admin/admin.php — $_GET['page'] in includes() gated behind current_user_can(); $_GET['plugin-boilerplate-install-plugin-redirect'] now uses sanitize_key(wp_unslash()) fixing MissingUnslash and InputNotSanitized; $_GET['page'] and $_GET['activate-multi'] in admin_redirects() gated behind current_user_can()
* FIXED: includes/admin/views/html-admin-page.php — GET reads gated behind current_user_can(); isset() added for seedview fixing InputNotValidated; sanitize_key(wp_unslash()) applied fixing MissingUnslash and InputNotSanitized; loop variables renamed to $plugin_boilerplate_key/$plugin_boilerplate_report_group/$plugin_boilerplate_links
...
* FIXED: includes/classes/ajax.php — nonce extracted into sanitised local variable fixing MissingUnslash and InputNotSanitized; wp_unslash() added to plugin-boilerplate-ajax value; stale ini_set() and NonceVerification results removed (already gated behind WP_DEBUG)
* FIXED: includes/classes/ajax.php — $_GET['plugin-boilerplate-ajax'] in define_ajax() extracted into $plugin_boilerplate_ajax_action via sanitize_text_field(wp_unslash()) immediately on read, satisfying WordPress.Security.NonceVerification.Recommended; PHPDoc added to init() and define_ajax(); file header @version bumped to 2.0.0
...
* FIXED: includes/classes/install.php — install_action_do_update() now verifies nonce and current_user_can() before running; install_action_updater_cron() gated behind current_user_can(); hook renamed from wp_plugin_boilerplate_updater_cron to plugin_boilerplate_updater_cron; stale DirectQuery/NoCaching results removed (one-time activation query)
...
* FIXED: Global variable $progress_script renamed to $plugin_boilerplate_progress_script in progress-indicators.php to satisfy prefix naming standard
* FIXED: Global variable $status_script renamed to $plugin_boilerplate_status_script in status-indicators.php to satisfy prefix naming standard
...
* FIXED: Global variable $security_checks renamed to $plugin_boilerplate_security_checks in security-audit.php to satisfy prefix naming standard
