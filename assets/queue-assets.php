...
class EvolveWP_Core_Asset_Queue {
    ...
    public function __construct() {
        ...
        $this->asset_manager = $plugin_boilerplate_assets;
    ...
    private function enqueue_style($name, $asset) {
        ...
        $deps = array_map( function( $dep ) {
            ...
            if ( 0 === strpos( $dep, 'plugin-boilerplate-' ) || 0 === strpos( $dep, 'wp-' ) ) {
                return $dep;
            }
            return 'plugin-boilerplate-' . $dep;
        }, $asset['dependencies'] );
    ...
    private function enqueue_script($name, $asset) {
        ...
        // Prefix dependency handles to match registered handle format.
        $deps = array_map( function( $dep ) {
            if ( 0 === strpos( $dep, 'plugin-boilerplate-' ) || 0 === strpos( $dep, 'wp-' ) || 'jquery' === $dep ) {
                return $dep;
            }
            return 'plugin-boilerplate-' . $dep;
        }, $asset['dependencies'] );
