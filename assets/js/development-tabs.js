/**
 * plugin-boilerplate Development Tabs JavaScript
 *
...
(function($) {
    ...
    function initdevelopment() {
        console.log('plugin-boilerplate Development Tabs JS loaded');
    ...
    function initAccordions() {
        ...
        if ($('.plugin-boilerplate-accordion-container').length > 0) {
            ...
            $('.collapse-all').off('click').on('click', function() {
                $('.plugin-boilerplate-accordion-content').slideUp();
                $('.plugin-boilerplate-accordion-icon')
    ...
    function initUILibrary() {
        ...
        if (typeof plugin-boilerplateUILibrary !== 'undefined') {
            // UI Library JavaScript is loaded
            console.log('plugin-boilerplateUILibrary object detected');
        }
    }
...
jQuery(document).ready(function($) {
    ...
    $('.refresh-github-data').on('click', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'plugin_boilerplate_refresh_github_data',
                nonce: plugin_boilerplate_dev_tabs.nonce
            },
    ...
    $('.section-toggle-checkbox').on('change', function() {
        ...
        localStorage.setItem('plugin_boilerplate_ui_library_sections', JSON.stringify(sectionStates));
    });
    ...
    var savedStates = localStorage.getItem('plugin_boilerplate_ui_library_sections');
    if (savedStates) {
        try {
    ...
    $('.move-to-assets').on('click', function() {
        ...
        if (confirm(plugin-boilerplateData.confirmMoveFile || 'Are you sure you want to move this file to the assets folder?')) {
            $btn.prop('disabled', true).text(plugin-boilerplateData.movingText || 'Moving...');
            
            setTimeout(function() {
                alert(plugin-boilerplateData.moveFileMessage || 'File move functionality would be implemented here');
                $btn.prop('disabled', false).text(plugin-boilerplateData.moveToAssetsText || 'Move to Assets');
            }, 1000);
        }
    });
    ...
    $('.delete-file').on('click', function() {
        ...
        if (confirm(plugin-boilerplateData.confirmDeleteFile || 'Are you sure you want to delete this file? This action cannot be undone.')) {
            $btn.prop('disabled', true).text(plugin-boilerplateData.deletingText || 'Deleting...');
            
            setTimeout(function() {
                alert(plugin-boilerplateData.deleteFileMessage || 'File delete functionality would be implemented here');
                $btn.prop('disabled', false).text(plugin-boilerplateData.deleteText || 'Delete');
            }, 1000);
        }
    });
    ...
    if ($('.feature-status-tab').length > 0) {
        ...
        // Initialize plugin-boilerplate-accordion functionality for feature status
        $('.plugin-boilerplate-accordion-header').on('click', function() {
            var $content = $(this).next('.plugin-boilerplate-accordion-content');
            $content.slideToggle();
            $(this).find('.plugin-boilerplate-accordion-icon')
    ...
    $('#load-log-file').on('click', function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'plugin_boilerplate_load_log_file',
                log_file: logFile,
                security: plugin_boilerplate_dev_tabs.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Tasks functionality
    if (typeof plugin-boilerplateTasksData !== 'undefined') {
        $('.start-task').on('click', function(e) {
            e.preventDefault();
            var taskId = $(this).data('task-id');
            var taskSource = $(this).data('task-source');
            
            localStorage.setItem('plugin_boilerplate_current_task_id', taskId);
            localStorage.setItem('plugin_boilerplate_current_task_source', taskSource);
            
            $(this).closest('tr').attr('data-status', 'in-progress');
            
            if (taskSource !== 'github') {
                var taskData = null;
                for (var i = 0; i < plugin-boilerplateTasksData.tasks.length; i++) {
                    if (plugin-boilerplateTasksData.tasks[i].id === taskId) {
                        taskData = plugin-boilerplateTasksData.tasks[i];
                        break;
                    }
                }
                ...
                if (taskData) {
                    ...
                    $.ajax({
                        url: plugin_boilerplate_dev_tabs.ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'plugin_boilerplate_create_github_issue',
                            nonce: plugin_boilerplate_dev_tabs.nonce,
                        ...
                        success: function(response) {
                            if (response.success) {
                                localStorage.setItem('plugin_boilerplate_GITHUB_issue_number', response.data.number);
                                localStorage.setItem('plugin_boilerplate_GITHUB_issue_url', response.data.html_url);
                                window.location.href = 'admin.php?page=plugin_boilerplate_development&tab=current_task';
                            } else {
