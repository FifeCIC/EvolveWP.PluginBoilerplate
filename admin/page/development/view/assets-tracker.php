...
class EvolveWP_Boilerplate_Admin_Development_Assets {
    ...
    public static function output() {
        ...
        if (!$asset_manager) {
            echo '<div class="notice notice-error"><p>' . esc_html__('Asset manager not available.', 'plugin-boilerplate') . '</p></div>';
            return;
        }
        ...
        ?>
        <div class="plugin-boilerplate-assets-container">
            ...
            <h3><?php esc_html_e('CSS Assets', 'plugin-boilerplate'); ?></h3>
            ...
            <h3><?php esc_html_e('JavaScript Assets', 'plugin-boilerplate'); ?></h3>
    ...
    private static function get_asset_manager() {
        ...
        return $plugin_boilerplate_assets ?? false;
    }

    private static function render_asset_table($assets, $type) {
        if (empty($assets) || !is_array($assets)) {
            echo '<p>' . esc_html__('No assets found.', 'plugin-boilerplate') . '</p>';
            return;
        }
        ?>
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php esc_html_e('Name', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Category', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Purpose', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Status', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Path', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Pages', 'plugin-boilerplate'); ?></th>
                    <th><?php esc_html_e('Dependencies', 'plugin-boilerplate'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php
    ...
    private static function check_asset_status($asset, $type) {
        if (!is_array($asset) || empty($asset['path'])) {
            return array('type' => 'error', 'icon' => 'dashicons-no', 'message' => __('Invalid', 'plugin-boilerplate'));
        }
        
        if ($asset['path'] === 'external') {
            return array('type' => 'success', 'icon' => 'dashicons-yes', 'message' => __('External', 'plugin-boilerplate'));
        }
        
        $asset_path = PLUGIN_BOILERPLATE_PLUGIN_DIR . 'assets/' . ltrim($asset['path'], '/');
        
        if (file_exists($asset_path)) {
            return array('type' => 'success', 'icon' => 'dashicons-yes', 'message' => __('Available', 'plugin-boilerplate'));
        } else {
            return array('type' => 'error', 'icon' => 'dashicons-no', 'message' => __('Missing', 'plugin-boilerplate'));
        }
    }
    ...
    private static function check_overall_status($css_assets, $js_assets) {
        ...
        if ($total_missing === 0) {
            return array(
                ...
                'icon' => 'dashicons-yes',
                'message' => __('All Assets Available', 'plugin-boilerplate'),
                /* translators: %d: Number of assets managed */
                'details' => sprintf(__('%1$d assets managed', 'plugin-boilerplate'), $total_assets)
            );
        } elseif ($total_missing <= 3) {
            return array(
                'class' => 'status-warning',
                'icon' => 'dashicons-warning',
                'message' => __('Some Assets Missing', 'plugin-boilerplate'),
                /* translators: %1$d: Number of missing assets, %2$d: Total number of assets */
                'details' => sprintf(__('%1$d of %2$d missing', 'plugin-boilerplate'), $total_missing, $total_assets)
            );
        } else {
            return array(
                'class' => 'status-error',
                'icon' => 'dashicons-no',
                'message' => __('Many Assets Missing', 'plugin-boilerplate'),
                /* translators: %1$d: Number of missing assets, %2$d: Total number of assets */
                'details' => sprintf(__('%1$d of %2$d missing', 'plugin-boilerplate'), $total_missing, $total_assets)
            );
        }
